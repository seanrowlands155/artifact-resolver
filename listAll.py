...
scriptname = 'listrepo2'
scriptlocation = 'testscript.groovy'
DEBUG = 'false'
...
statuscode, statusmessage, header, res = do_http_request(NEXUS_HOST, NEXUS_PORT,
                                                         '/service/siesta/rest/v1/script/' + scriptname, 'DELETE',
                                                         'application/json', NEXUS_USERNAME, NEXUS_PASSWORD, '')
if statuscode not in [204, 404]:
    raise Exception('HTTP request failed',
                    'Expected HTTP status 204 or 404 as response to DELETE request to remove current script. Got: ' + str(
                        statuscode))
log('DEBUG', 'Deleted current script: ' + str(scriptname))

# create a JSON request for the API
request = '{"name": "' + str(scriptname) + '","type": "groovy","content": "' + process_file_to_single_line(
    scriptlocation) + '"}'
print "Request: \n" + request
log('DEBUG', 'Request: ' + str(request))

# upload a new version of the script
statuscode, statusmessage, header, res = do_http_request(NEXUS_HOST, NEXUS_PORT, '/service/siesta/rest/v1/script/',
                                                         'POST', 'application/json', NEXUS_USERNAME, NEXUS_PASSWORD,
                                                         request)
if statuscode is not 204:
    raise Exception('HTTP request failed',
                    'Expected HTTP status 204 as response to POST request to upload new script. Got: ' + str(
                        statuscode))
...
statuscode, statusmessage, header, res = do_http_request(NEXUS_HOST, NEXUS_PORT,
                                                         ...
                                                         'POST', 'text/plain', NEXUS_USERNAME, NEXUS_PASSWORD, '')
if statuscode is not 200:
    raise Exception('HTTP request failed',
                    'Expected HTTP status 200 as response to POST request to call the new script. Got: ' + str(
                        statuscode))
